...
import { Heart, Plus, ShieldCheck, Lock, Check, Send, Filter, X, Sparkles, MessageSquare } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-800 pb-8">
          <div className="space-y-3 max-w-2xl">
            ...
            <p className="text-slate-400 text-sm sm:text-base leading-relaxed">
              {lang === 'zh' 
                ? '在愛中互相擔當重擔。您可以點擊「我為此禱告」，或提交個人代禱需求（可選擇僅陳嘉彰牧師密件收悉）。'
              ...
              {submitSuccess ? (
                <div className="p-6 bg-emerald-950/80 border border-emerald-600/50 rounded-2xl text-center space-y-2">
                  ...
                  <p className="text-xs text-emerald-300">
                    {isConfidential 
                      ? (lang === 'zh' ? '此代禱已加密寄送給陳嘉彰牧師，牧師將為您私下守望禱告。' : 'Sent privately to Rev. Chen Jiachang.')
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="flex items-end pb-1">
                      <label className="flex items-center space-x-2 text-xs text-amber-300 cursor-pointer">
                        ...
                        />
                        <span>{lang === 'zh' ? '牧師保密代禱 (不公開)' : 'Confidential to Pastor'}</span>
                      </label>
                    </div>
                  </div>
