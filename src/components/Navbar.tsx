...
import { Church, Globe, Heart, Phone, MapPin, Calendar, Clock, Menu, X, Sparkles, FileText, ShieldCheck, LogOut, Lock } from 'lucide-react';
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <a href="#" className="flex items-center space-x-3 group">
            <div className={`p-2.5 rounded-xl transition-transform group-hover:scale-105 shadow-sm ${
              scrolled ? 'bg-amber-700 text-white' : 'bg-amber-600 text-white border border-amber-400/30'
            }`}>
              <Church className="w-6 h-6" />
            </div>
            <div>
              <div className={`font-serif text-lg sm:text-xl font-bold tracking-tight leading-tight ${
          ...
          <div className="hidden sm:flex items-center space-x-3">
            <button
              ...
              className="flex items-center space-x-1.5 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white px-4 py-2 rounded-lg font-semibold text-sm shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5"
            >
              <Heart className="w-4 h-4 fill-white/20" />
              <span>{lang === 'zh' ? '奉獻支持' : 'Online Give'}</span>
            </button>
