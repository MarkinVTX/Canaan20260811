...
import { Users, Music, BookOpen, HeartHandshake, HandHeart, Clock, MapPin, CheckCircle, Send, X, ArrowRight } from 'lucide-react';
              ...
              {joinSubmitted ? (
                <div className="p-6 bg-emerald-50 border border-emerald-200 rounded-2xl text-center space-y-2">
                  ...
                  <div className="font-bold text-emerald-900 text-base">
                    {lang === 'zh' ? '感謝您的參與需求！' : 'Thank you for connecting!'}
                  </div>
                  <p className="text-xs text-emerald-800">
                    {lang === 'zh' 
                      ? '教會同工或團契輔導將儘速與您聯繫，願上帝賜福給您！'
                      : 'Our ministry team will reach out to you shortly. May God bless you!'}
                  </p>
                </div>
              ) : (
                <form onSubmit={handleJoinSubmit} className="space-y-4 text-xs sm:text-sm">
                  ...
                  <button
                    type="submit"
                    className="w-full flex items-center justify-center space-x-2 bg-amber-700 hover:bg-amber-800 text-white font-bold py-3 rounded-xl shadow-md transition-colors"
                  >
                    <Send className="w-4 h-4" />
                    <span>{lang === 'zh' ? '送出參與意願' : 'Submit Connection Request'}</span>
                  </button>
                </form>
              )}
